...
const App: React.FC = () => {
  ...
  const menuItems = [
    ...
    { id: 'guide', label: 'Guide', icon: 'M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253' }
  ];

  const ThemeToggle = () => (
    <button
      onClick={() => setIsDark(!isDark)}
      className="p-2 rounded-xl bg-slate-200 dark:bg-slate-800 text-slate-700 dark:text-yellow-400 hover:scale-110 active:scale-95 transition-all shadow-sm print:hidden"
    >
      {isDark ? (
  ...
  return (
    <div className="flex flex-col md:flex-row min-h-screen bg-slate-50 dark:bg-[#0f172a] text-slate-900 dark:text-slate-200 font-sans transition-colors duration-300">
      <aside className="w-64 bg-slate-100 dark:bg-[#1e293b]/50 border-r border-slate-200 dark:border-slate-800 flex-col hidden md:flex h-screen sticky top-0 print:hidden">
      ...
      <header className="md:hidden py-3 px-4 border-b border-slate-200 dark:border-slate-800 bg-white/80 dark:bg-[#1e293b]/50 sticky top-0 z-50 flex justify-between items-center backdrop-blur-lg print:hidden">
      ...
      <div className="flex-1 flex flex-col h-screen overflow-hidden print:h-auto print:overflow-visible">
        <header className="hidden md:flex items-center justify-end px-8 py-3 bg-white/40 dark:bg-[#0f172a]/40 border-b border-slate-200 dark:border-slate-800 backdrop-blur-sm shrink-0 print:hidden">
          <div className="flex items-center space-x-4">
             <ThemeToggle />
          </div>
        </header>

        <main ref={mainRef} className="flex-1 overflow-y-auto p-4 md:p-8 pb-24 md:pb-8 print:p-0 print:overflow-visible">
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-white/95 dark:bg-[#1e293b]/95 border-t border-slate-200 dark:border-slate-800 flex justify-around items-center px-1 py-1.5 pb-4 z-50 backdrop-blur-md print:hidden">
        {menuItems.map(item => (
          <button
            key={item.id}
            onClick={() => setView(item.id as ViewType)}
